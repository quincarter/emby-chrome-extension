...
describe("buildServerItemUrl", () => {
  it("builds Emby URL", () => {
    ...
    expect(url).toBe(
      "https://emby.example.com/web/index.html#!/item?id=abc123",
    );
  });
  ...
  it("builds Emby URL with server ID", () => {
    ...
    expect(url).toBe(
      "https://emby.example.com/web/index.html#!/item?id=abc123&serverId=server-1",
    );
  });
  ...
  it("builds Jellyfin URL with server ID", () => {
    ...
    expect(url).toBe(
      "https://jf.example.com/web/#/details?id=xyz789&serverId=server-2",
    );
  });
});
